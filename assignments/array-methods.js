...
// Problem 1
// How many people want medium shirts?
let medShirtsSum = runners.filter((arr) => {
    return arr.shirt_size == 'M';
}).length;

console.log(medShirtsSum);

// Problem 2
// Format an array that has the full name in caps and their ID number for printing. Example result: {'name': 'GEORGE LOPEZ', 'id': 10}
let printArray = runners.map((arr) => {
    return {'name': arr.first_name.toUpperCase() + ' ' + arr.last_name.toUpperCase(), 'id': arr.id};
})

console.log(printArray);

// Problem 3
// Give me an array of each shirt size and how many we need to order.

let shirtSizes = runners.map((arr) => {
...
for (let i = 0; i < shirtSizes.length; i++) {
    if (!shirtNoDupes.includes(shirtSizes[i])) {
        shirtNoDupes.push(shirtSizes[i]);
    }
}
shirtNoDupes.sort();
...
for (let i = 0; i < shirtNoDupes.length; i++) {
    let count = 0;
    for (let n = 0; n < shirtSizes.length; n++) {
        if (shirtNoDupes[i] == shirtSizes[n]) {
            count++;
        }
    }
